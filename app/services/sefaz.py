...
Suporta todos os tipos de CNPJ:
  ...
  - Código de acesso e-CAC: sem certificado, MEI/ME com código gerado no portal
  - Certificado A1 (.pfx): acesso completo, DFe, manifestação
  - CPF: consulta como pessoa física
"""
...
ENDPOINTS = {
    "homologacao": {
        ...
        "nfe_consulta_publica":  "https://www.nfe.fazenda.gov.br/portal/consultaRecaptcha.aspx",
    },
    "producao": {
...
AUTH_NONE        = "none"          # sem auth — consulta pública por chave
AUTH_CODIGO      = "codigo_acesso" # código de acesso e-CAC (MEI/ME sem certificado)
AUTH_CERTIFICADO = "certificado"   # certificado A1 (.pfx)
...
class SefazService:
    ...
    Modos de operação:
    1. AUTH_NONE        — consulta pública por chave de acesso (qualquer empresa)
    2. AUTH_CODIGO      — código de acesso e-CAC (MEI/ME sem certificado)
    3. AUTH_CERTIFICADO — certificado A1 em bytes (empresas com e-CNPJ A1)
    """
...
<soapenv:Envelope
  ...
  <soapenv:Body>
    ...
    # ═══════════════════════════════════════════════════════════
    # MODO 1 — CONSULTA PÚBLICA POR CHAVE (sem autenticação)
    # Funciona para: MEI, qualquer CNPJ, qualquer pessoa
    # ═══════════════════════════════════════════════════════════
    async def consulta_nfe_chave(self, chave: str) -> SefazResult:
        """
        Consulta NF-e por chave de acesso.
        NÃO precisa de certificado — funciona para qualquer empresa.
        """
        tp_amb = 1 if self.ambiente == "producao" else 2

        if self.auth_mode == AUTH_CERTIFICADO:
            # Com certificado: webservice completo com XML
  ...
  <tpAmb>{tp_amb}</tpAmb><xServ>CONSULTAR</xServ><chNFe>{chave}</chNFe>
            ...
            try:
                ...
                log.exception("Erro consultando NF-e com certificado chave=%s", chave)
                return SefazResult(success=False, error=str(exc), auth_mode=AUTH_CERTIFICADO)
        else:
            # Sem certificado: webservice público de consulta (retorna dados básicos)
            body = f"""<consSitNFe xmlns="http://www.portalfiscal.inf.br/nfe" versao="4.00">
  <tpAmb>{tp_amb}</tpAmb><xServ>CONSULTAR</xServ><chNFe>{chave}</chNFe>
</consSitNFe>"""
            try:
                # A SEFAZ permite consulta de status sem certificado no webservice público
                ...
                log.exception("Erro consultando NF-e pública chave=%s", chave)
                # Fallback: cria documento básico com dados da chave
                return self._doc_from_chave(chave)

    def _doc_from_chave(self, chave: str) -> SefazResult:
        """Extrai dados básicos da chave de acesso sem consultar a SEFAZ."""
        try:
            # A chave NF-e tem estrutura definida: cUF(2) AAMM(4) CNPJ(14) mod(2) serie(3) nNF(9) tpEmis(1) cNF(8) cDV(1)
            if len(chave) != 44:
    ...
    # ═══════════════════════════════════════════════════════════
    # MODO 2 — CÓDIGO DE ACESSO e-CAC (MEI/ME sem certificado)
    # O usuário gera um código em https://cav.receita.fazenda.gov.br
    # ═══════════════════════════════════════════════════════════
    async def distribuicao_dfe_codigo_acesso(
        ...
        """
        Consulta DFe usando código de acesso e-CAC.
        Para MEI e microempresas que não possuem certificado digital.
        
        Como obter o código:
        1. Acesse https://cav.receita.fazenda.gov.br
        2. Login com CPF + senha da Receita Federal
        3. Vá em "Código de Acesso" → Gerar
        4. Use o código de 8 dígitos gerado
        """
  ...
  <distNSU><ultNSU>{ult_nsu}</ultNSU></distNSU>
    ...
    # ═══════════════════════════════════════════════════════════
    # MODO 3 — CERTIFICADO A1 (acesso completo)
    # ═══════════════════════════════════════════════════════════
    async def distribuicao_dfe(
        ...
        """
        Consulta DFe pelo CNPJ do destinatário.
        ...
        if self.auth_mode != AUTH_CERTIFICADO:
            return SefazResult(
                ...
                error="Certificado digital A1 necessário para consulta por CNPJ. "
                      "Para MEI/ME use o código de acesso e-CAC.",
  ...
  <distNSU><ultNSU>{ult_nsu}</ultNSU></distNSU>
    ...
    # Requer certificado OU código de acesso
    # ═══════════════════════════════════════════════════════════
